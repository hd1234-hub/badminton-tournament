      ...
      <div className="card-nailong p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
            🏟️ 我的俱乐部
            {clubs.length > 0 && (
              <span className="text-sm font-normal text-gray-400">({clubs.length})</span>
            )}
          </h2>
          {clubs.length > 3 && (
            <button
              onClick={() => setShowAllClubs(!showAllClubs)}
              className="text-sm text-nailong-orange hover:text-nailong-orange-dark font-medium transition"
            >
              {showAllClubs ? "收起 ↑" : `展开全部 (${clubs.length}) ↓`}
            </button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 bg-nailong-cream/50 rounded-2xl">
            <p className="text-gray-500">还没有俱乐部</p>
            <p className="text-nailong-orange text-sm mt-1">创建一个或在下方搜索加入</p>
          </div>
        ) : (
          <div className="space-y-2">
            {(showAllClubs ? clubs : clubs.slice(0, 3)).map(club => (
              <Link
                key={club.id}
                to={`/clubs/${club.id}`}
                className="flex items-center gap-3 p-3 rounded-xl bg-nailong-cream hover:bg-nailong-cream-dark transition group"
              >
                <div className="text-2xl">🏟️</div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-medium text-gray-800 group-hover:text-nailong-orange transition truncate">
                    {club.name}
                  </h3>
                ...
                <span className="text-gray-300 group-hover:text-nailong-orange transition">→</span>
              </Link>
            ))}
            {!showAllClubs && clubs.length > 3 && (
              <button
                onClick={() => setShowAllClubs(true)}
                className="w-full py-2 text-sm text-gray-400 hover:text-nailong-orange transition bg-gray-50 hover:bg-nailong-cream rounded-xl"
              >
                还有 {clubs.length - 3} 个俱乐部，点击展开 ↓
              </button>
            )}
          </div>
        )}
...
with io.open("../src/pages/Dashboard.tsx", "w", encoding="utf-8") as f:
    f.write(dashboard_code)

print("Dashboard v2 generated with collapsible clubs list")
