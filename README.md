...
支持公开大厅赛创建、俱乐部管理、热门推荐与 AI 助手入口，适合作为赛事系统的统一入口。

![赛事大厅](docs/images/dashboard.png)

#### 比赛计分看板

支持实时计分、排名更新、胜率预测、结束比赛与分享。现场比赛时可直接在场地上录入比分，系统自动刷新胜负场与净胜分。

![比赛计分看板](docs/images/competition-board.png)

#### 排行榜

按俱乐部维度统计胜场、负场、胜率与净胜分，支持切换不同俱乐部查看战绩，适合长期运营和数据沉淀。

![排行榜](docs/images/leaderboard.png)

#### AI 助手（核心能力）

通过自然语言完成赛事操作，不仅支持聊天，还能直接调用后端工具执行任务。典型能力包括：

- **一句话创建比赛**：如「帮我创建一个八人转」
- **创建比赛 + 录入比分一次完成**：如「帮我创建一场和 hd2 的比赛，比分为 21 比 0」，AI 会先创建比赛，再自动写入比分
- **查询俱乐部 / 比赛 / 球员信息**
- **辅助记录比分、查询赛况、推荐玩法**
- **结合上下文记忆**，按当前俱乐部继续对话
...
基于球员信息生成趣味对战分析，增强比赛互动感。除预测外，AI 还可配合比赛看板做比分记录、赛事查询、俱乐部运营建议等扩展能力。

![毒奶预测](docs/images/ai-prediction.png)
...
- **排行榜**：俱乐部维度战绩统计
- **AI 助手**（可选）：自然语言创建比赛、查询赛况、录入比分；支持「创建比赛 + 写比分」一步完成（需配置 Anthropic 兼容 API）
- **管理后台**：赛事与用户统计
